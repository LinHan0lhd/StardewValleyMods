using System;
using Microsoft.Xna.Framework;
using StardewValley.Audio;

namespace StardewValley.Tools
{
	// Token: 0x02000131 RID: 305
	public class Raft : Tool
	{
		// Token: 0x06001898 RID: 6296 RVA: 0x00122561 File Offset: 0x00120761
		public Raft() : base("Raft", 0, 1, 1, false, 0)
		{
			base.InstantUse = true;
		}

		// Token: 0x06001899 RID: 6297 RVA: 0x0012257A File Offset: 0x0012077A
		protected override Item GetOneNew()
		{
			return new Raft();
		}

		// Token: 0x0600189A RID: 6298 RVA: 0x00122581 File Offset: 0x00120781
		protected override string loadDisplayName()
		{
			return Game1.content.LoadString("Strings\\StringsFromCSFiles:Raft.cs.14204");
		}

		// Token: 0x0600189B RID: 6299 RVA: 0x00122592 File Offset: 0x00120792
		protected override string loadDescription()
		{
			return Game1.content.LoadString("Strings\\StringsFromCSFiles:Raft.cs.14205");
		}

		// Token: 0x0600189C RID: 6300 RVA: 0x001225A4 File Offset: 0x001207A4
		public override void DoFunction(GameLocation location, int x, int y, int power, Farmer who)
		{
			base.DoFunction(location, x, y, power, who);
			if (!who.isRafting && location.isWaterTile(x / 64, y / 64))
			{
				who.isRafting = true;
				Rectangle collidingBox = new Rectangle(x - 32, y - 32, 64, 64);
				if (location.isCollidingPosition(collidingBox, Game1.viewport, who))
				{
					who.isRafting = false;
					return;
				}
				who.xVelocity = ((who.FacingDirection == 1) ? 3f : ((who.FacingDirection == 3) ? -3f : 0f));
				who.yVelocity = ((who.FacingDirection == 2) ? 3f : ((who.FacingDirection == 0) ? -3f : 0f));
				who.Position = new Vector2((float)(x - 32), (float)(y - 32 - 32 - ((y < who.StandingPixel.Y) ? 64 : 0)));
				if (this.PlayUseSounds)
				{
					who.playNearbySoundAll("dropItemInWater", null, SoundContext.Default);
				}
			}
			base.CurrentParentTileIndex = base.IndexOfMenuItemView;
		}
	}
}
