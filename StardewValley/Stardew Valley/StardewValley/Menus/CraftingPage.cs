using System;
using System.Collections.Generic;
using System.Linq;
using Microsoft.Xna.Framework;
using Microsoft.Xna.Framework.Graphics;
using Microsoft.Xna.Framework.Input;
using Netcode;
using StardewValley.Buffs;
using StardewValley.Extensions;
using StardewValley.GameData.Objects;
using StardewValley.Inventories;
using StardewValley.ItemTypeDefinitions;
using StardewValley.Network;
using StardewValley.Quests;

namespace StardewValley.Menus
{
	// Token: 0x02000264 RID: 612
	public class CraftingPage : IClickableMenu
	{
		// Token: 0x06002896 RID: 10390 RVA: 0x001D98F8 File Offset: 0x001D7AF8
		public CraftingPage(int x, int y, int width, int height, bool cooking = false, bool standaloneMenu = false, List<IInventory> materialContainers = null) : base(x, y, width, height, false)
		{
			this._standaloneMenu = standaloneMenu;
			this.cooking = cooking;
			this.inventory = new InventoryMenu(this.xPositionOnScreen + IClickableMenu.spaceToClearSideBorder + IClickableMenu.borderWidth, this.yPositionOnScreen + IClickableMenu.spaceToClearTopBorder + IClickableMenu.borderWidth + 320 - 16, false, null, null, -1, 3, 0, 0, true);
			this.inventory.showGrayedOutSlots = true;
			this.currentPageClickableComponents = new List<ClickableComponent>();
			foreach (ClickableComponent clickableComponent in this.inventory.GetBorder(InventoryMenu.BorderSide.Top))
			{
				clickableComponent.upNeighborID = -99998;
			}
			this._materialContainers = materialContainers;
			if (this._standaloneMenu)
			{
				base.initializeUpperRightCloseButton();
			}
			this.trashCan = new ClickableTextureComponent(new Rectangle(this.xPositionOnScreen + width + 4, this.yPositionOnScreen + height - 192 - 32 - IClickableMenu.borderWidth - 104, 64, 104), Game1.mouseCursors, new Rectangle(564 + Game1.player.trashCanLevel * 18, 102, 18, 26), 4f, false)
			{
				myID = 106
			};
			this.dropItemInvisibleButton = new ClickableComponent(new Rectangle(this.xPositionOnScreen - IClickableMenu.borderWidth - IClickableMenu.spaceToClearSideBorder - 64, this.trashCan.bounds.Y, 64, 64), "")
			{
				myID = 107,
				rightNeighborID = 0
			};
			if (this._standaloneMenu)
			{
				Game1.playSound("bigSelect", null);
			}
			this.RepositionElements();
			if (Game1.options.SnappyMenus)
			{
				this.snapToDefaultClickableComponent();
			}
		}

		// Token: 0x06002897 RID: 10391 RVA: 0x001D9AF4 File Offset: 0x001D7CF4
		protected virtual List<string> GetRecipesToDisplay()
		{
			List<string> recipes = new List<string>();
			if (!this.cooking)
			{
				using (Dictionary<string, string>.KeyCollection.Enumerator enumerator = CraftingRecipe.craftingRecipes.Keys.GetEnumerator())
				{
					while (enumerator.MoveNext())
					{
						string key = enumerator.Current;
						if (Game1.player.craftingRecipes.ContainsKey(key))
						{
							recipes.Add(key);
						}
					}
					return recipes;
				}
			}
			foreach (string key2 in CraftingRecipe.cookingRecipes.Keys)
			{
				if (!key2.Equals("Moss Soup"))
				{
					recipes.Add(key2);
				}
			}
			recipes.Sort(delegate(string a, string b)
			{
				int indexA = -1;
				int indexB = -1;
				string rawDataA;
				if (a != null && CraftingRecipe.cookingRecipes.TryGetValue(a, out rawDataA))
				{
					indexA = ArgUtility.GetInt(rawDataA.Split('/', StringSplitOptions.None), 2, -1);
				}
				string rawDataB;
				if (b != null && CraftingRecipe.cookingRecipes.TryGetValue(b, out rawDataB))
				{
					indexB = ArgUtility.GetInt(rawDataB.Split('/', StringSplitOptions.None), 2, -1);
				}
				return indexA.CompareTo(indexB);
			});
			recipes.Add("Moss Soup");
			return recipes;
		}

		// Token: 0x06002898 RID: 10392 RVA: 0x001D9BF4 File Offset: 0x001D7DF4
		protected virtual IList<Item> getContainerContents()
		{
			if (this._materialContainers == null)
			{
				return null;
			}
			List<Item> items = new List<Item>();
			foreach (IInventory container in this._materialContainers)
			{
				items.AddRange(container);
			}
			return items;
		}

		// Token: 0x06002899 RID: 10393 RVA: 0x001D9C58 File Offset: 0x001D7E58
		private int craftingPageY()
		{
			return this.yPositionOnScreen + IClickableMenu.spaceToClearTopBorder + IClickableMenu.borderWidth - 16;
		}

		// Token: 0x0600289A RID: 10394 RVA: 0x001D9C6F File Offset: 0x001D7E6F
		private ClickableTextureComponent[,] createNewPageLayout()
		{
			return new ClickableTextureComponent[10, 4];
		}

		// Token: 0x0600289B RID: 10395 RVA: 0x001D9C7C File Offset: 0x001D7E7C
		private Dictionary<ClickableTextureComponent, CraftingRecipe> createNewPage()
		{
			Dictionary<ClickableTextureComponent, CraftingRecipe> page = new Dictionary<ClickableTextureComponent, CraftingRecipe>();
			this.pagesOfCraftingRecipes.Add(page);
			return page;
		}

		// Token: 0x0600289C RID: 10396 RVA: 0x001D9C9C File Offset: 0x001D7E9C
		private bool spaceOccupied(ClickableTextureComponent[,] pageLayout, int x, int y, CraftingRecipe recipe)
		{
			return pageLayout[x, y] != null || (recipe.bigCraftable && (y + 1 >= 4 || pageLayout[x, y + 1] != null));
		}

		// Token: 0x0600289D RID: 10397 RVA: 0x001D9CCC File Offset: 0x001D7ECC
		private void layoutRecipes(List<string> playerRecipes)
		{
			int craftingPageX = this.xPositionOnScreen + IClickableMenu.spaceToClearSideBorder + IClickableMenu.borderWidth - 16;
			int spaceBetweenCraftingIcons = 8;
			Dictionary<ClickableTextureComponent, CraftingRecipe> currentPage = this.createNewPage();
			int x = 0;
			int y = 0;
			int i = 0;
			ClickableTextureComponent[,] pageLayout = this.createNewPageLayout();
			List<ClickableTextureComponent[,]> pageLayouts = new List<ClickableTextureComponent[,]>();
			pageLayouts.Add(pageLayout);
			foreach (string name in playerRecipes)
			{
				i++;
				CraftingRecipe recipe = new CraftingRecipe(name, this.cooking);
				while (this.spaceOccupied(pageLayout, x, y, recipe))
				{
					x++;
					if (x >= 10)
					{
						x = 0;
						y++;
						if (y >= 4)
						{
							currentPage = this.createNewPage();
							pageLayout = this.createNewPageLayout();
							pageLayouts.Add(pageLayout);
							x = 0;
							y = 0;
						}
					}
				}
				int id = 200 + i;
				ParsedItemData itemData = recipe.GetItemData(true);
				Texture2D texture = itemData.GetTexture();
				Rectangle sourceRect = itemData.GetSourceRect(0, null);
				ClickableTextureComponent component = new ClickableTextureComponent("", new Rectangle(craftingPageX + x * (64 + spaceBetweenCraftingIcons), this.craftingPageY() + y * 72, 64, recipe.bigCraftable ? 128 : 64), null, (this.cooking && !Game1.player.cookingRecipes.ContainsKey(recipe.name)) ? "ghosted" : "", texture, sourceRect, 4f, false)
				{
					myID = id,
					rightNeighborID = -99998,
					leftNeighborID = -99998,
					upNeighborID = -99998,
					downNeighborID = -99998,
					fullyImmutable = true,
					region = 8000
				};
				currentPage.Add(component, recipe);
				pageLayout[x, y] = component;
				if (recipe.bigCraftable)
				{
					pageLayout[x, y + 1] = component;
				}
			}
		}

		// Token: 0x0600289E RID: 10398 RVA: 0x001D9ECC File Offset: 0x001D80CC
		protected override void noSnappedComponentFound(int direction, int oldRegion, int oldID)
		{
			base.noSnappedComponentFound(direction, oldRegion, oldID);
			if (oldRegion == 8000 && direction == 2)
			{
				this.currentlySnappedComponent = base.getComponentWithID(oldID % 10);
				this.currentlySnappedComponent.upNeighborID = oldID;
			}
		}

		// Token: 0x0600289F RID: 10399 RVA: 0x001D9F00 File Offset: 0x001D8100
		public override void snapToDefaultClickableComponent()
		{
			this.currentlySnappedComponent = ((this.currentCraftingPage < this.pagesOfCraftingRecipes.Count) ? this.pagesOfCraftingRecipes[this.currentCraftingPage].First<KeyValuePair<ClickableTextureComponent, CraftingRecipe>>().Key : null);
			this.snapCursorToCurrentSnappedComponent();
		}

		// Token: 0x060028A0 RID: 10400 RVA: 0x001D9F50 File Offset: 0x001D8150
		protected override void actionOnRegionChange(int oldRegion, int newRegion)
		{
			base.actionOnRegionChange(oldRegion, newRegion);
			if (newRegion == 9000 && oldRegion != 0)
			{
				for (int i = 0; i < 10; i++)
				{
					if (this.inventory.inventory.Count > i)
					{
						this.inventory.inventory[i].upNeighborID = this.currentlySnappedComponent.upNeighborID;
					}
				}
			}
		}

		// Token: 0x060028A1 RID: 10401 RVA: 0x001D9FB4 File Offset: 0x001D81B4
		public override void receiveKeyPress(Keys key)
		{
			base.receiveKeyPress(key);
			if (key == Keys.Delete && this.heldItem != null && this.heldItem.canBeTrashed())
			{
				Utility.trashItem(this.heldItem);
				this.heldItem = null;
			}
			if (Game1.isAnyGamePadButtonBeingPressed() && Game1.options.doesInputListContain(Game1.options.menuButton, key) && this.heldItem != null)
			{
				Game1.setMousePosition(this.trashCan.bounds.Center);
			}
		}

		// Token: 0x060028A2 RID: 10402 RVA: 0x001DA030 File Offset: 0x001D8230
		public override void receiveScrollWheelAction(int direction)
		{
			base.receiveScrollWheelAction(direction);
			if (direction > 0 && this.currentCraftingPage > 0)
			{
				this.currentCraftingPage--;
				this._UpdateCurrentPageButtons();
				Game1.playSound("shwip", null);
				if (Game1.options.SnappyMenus)
				{
					this.setCurrentlySnappedComponentTo(88);
					this.snapCursorToCurrentSnappedComponent();
					return;
				}
			}
			else if (direction < 0 && this.currentCraftingPage < this.pagesOfCraftingRecipes.Count - 1)
			{
				this.currentCraftingPage++;
				this._UpdateCurrentPageButtons();
				Game1.playSound("shwip", null);
				if (Game1.options.SnappyMenus)
				{
					this.setCurrentlySnappedComponentTo(89);
					this.snapCursorToCurrentSnappedComponent();
				}
			}
		}

		// Token: 0x060028A3 RID: 10403 RVA: 0x001DA0EF File Offset: 0x001D82EF
		public override void gameWindowSizeChanged(Rectangle oldBounds, Rectangle newBounds)
		{
			base.gameWindowSizeChanged(oldBounds, newBounds);
			this.RepositionElements();
		}

		// Token: 0x060028A4 RID: 10404 RVA: 0x001DA100 File Offset: 0x001D8300
		public virtual void RepositionElements()
		{
			List<string> playerRecipes = this.GetRecipesToDisplay();
			this.pagesOfCraftingRecipes.Clear();
			this.layoutRecipes(playerRecipes);
			if (this.pagesOfCraftingRecipes.Count > 1)
			{
				this.upButton = new ClickableTextureComponent(new Rectangle(this.xPositionOnScreen + 768 + 32, this.craftingPageY(), 64, 64), Game1.mouseCursors, Game1.getSourceRectForStandardTileSheet(Game1.mouseCursors, 12, -1, -1), 0.8f, false)
				{
					myID = 88,
					downNeighborID = 89,
					rightNeighborID = 106,
					leftNeighborID = -99998
				};
				this.downButton = new ClickableTextureComponent(new Rectangle(this.xPositionOnScreen + 768 + 32, this.craftingPageY() + 192 + 32, 64, 64), Game1.mouseCursors, Game1.getSourceRectForStandardTileSheet(Game1.mouseCursors, 11, -1, -1), 0.8f, false)
				{
					myID = 89,
					upNeighborID = 88,
					rightNeighborID = 106,
					leftNeighborID = -99998
				};
			}
			this.inventory.SetPosition(this.xPositionOnScreen + IClickableMenu.spaceToClearSideBorder + IClickableMenu.borderWidth, this.yPositionOnScreen + IClickableMenu.spaceToClearTopBorder + IClickableMenu.borderWidth + 320 - 16);
			this.trashCan.bounds.X = this.xPositionOnScreen + this.width + 4;
			this.trashCan.bounds.Y = this.yPositionOnScreen + this.height - 192 - 32 - IClickableMenu.borderWidth - 104;
			this.dropItemInvisibleButton.bounds.X = this.xPositionOnScreen - IClickableMenu.borderWidth - IClickableMenu.spaceToClearSideBorder - 64;
			this.dropItemInvisibleButton.bounds.Y = this.trashCan.bounds.Y;
			if (this.upButton != null)
			{
				this.upButton.bounds.X = this.xPositionOnScreen + 768 + 32;
				this.upButton.bounds.Y = this.craftingPageY();
			}
			if (this.downButton != null)
			{
				this.downButton.bounds.X = this.xPositionOnScreen + 768 + 32;
				this.downButton.bounds.Y = this.craftingPageY() + 192 + 32;
			}
			this._UpdateCurrentPageButtons();
		}

		// Token: 0x060028A5 RID: 10405 RVA: 0x001DA360 File Offset: 0x001D8560
		public override void receiveLeftClick(int x, int y, bool playSound = true)
		{
			base.receiveLeftClick(x, y, true);
			this.heldItem = this.inventory.leftClick(x, y, this.heldItem, true);
			if (this.upButton != null && this.upButton.containsPoint(x, y) && this.currentCraftingPage > 0)
			{
				Game1.playSound("coin", null);
				this.currentCraftingPage = Math.Max(0, this.currentCraftingPage - 1);
				this._UpdateCurrentPageButtons();
				this.upButton.scale = this.upButton.baseScale;
			}
			if (this.downButton != null && this.downButton.containsPoint(x, y) && this.currentCraftingPage < this.pagesOfCraftingRecipes.Count - 1)
			{
				Game1.playSound("coin", null);
				this.currentCraftingPage = Math.Min(this.pagesOfCraftingRecipes.Count - 1, this.currentCraftingPage + 1);
				this._UpdateCurrentPageButtons();
				this.downButton.scale = this.downButton.baseScale;
			}
			foreach (ClickableTextureComponent c in this.pagesOfCraftingRecipes[this.currentCraftingPage].Keys)
			{
				int times = Game1.oldKBState.IsKeyDown(Keys.LeftShift) ? (Game1.oldKBState.IsKeyDown(Keys.LeftControl) ? 25 : 5) : 1;
				for (int i = 0; i < times; i++)
				{
					if (c.containsPoint(x, y, 4) && !c.hoverText.Equals("ghosted") && this.pagesOfCraftingRecipes[this.currentCraftingPage][c].doesFarmerHaveIngredientsInInventory(this.getContainerContents()))
					{
						this.clickCraftingRecipe(c, i == 0);
					}
				}
				if (this.heldItem != null && Game1.oldKBState.IsKeyDown(Keys.LeftShift) && this.heldItem.maximumStackSize() == 1 && Game1.player.couldInventoryAcceptThisItem(this.heldItem))
				{
					Game1.player.addItemToInventoryBool(this.heldItem, false);
					this.heldItem = null;
				}
			}
			if (this.trashCan != null && this.trashCan.containsPoint(x, y) && this.heldItem != null && this.heldItem.canBeTrashed())
			{
				Utility.trashItem(this.heldItem);
				this.heldItem = null;
				return;
			}
			if (this.heldItem != null && !this.isWithinBounds(x, y) && this.heldItem.canBeTrashed())
			{
				Game1.playSound("throwDownITem", null);
				Game1.createItemDebris(this.heldItem, Game1.player.getStandingPosition(), Game1.player.FacingDirection, null, -1, false);
				this.heldItem = null;
			}
		}

		// Token: 0x060028A6 RID: 10406 RVA: 0x001DA63C File Offset: 0x001D883C
		protected void _UpdateCurrentPageButtons()
		{
			this.currentPageClickableComponents.Clear();
			foreach (ClickableComponent component in this.pagesOfCraftingRecipes[this.currentCraftingPage].Keys)
			{
				this.currentPageClickableComponents.Add(component);
			}
			this.populateClickableComponentList();
		}

		// Token: 0x060028A7 RID: 10407 RVA: 0x001DA6B8 File Offset: 0x001D88B8
		private void clickCraftingRecipe(ClickableTextureComponent c, bool playSound = true)
		{
			CraftingRecipe recipe = this.pagesOfCraftingRecipes[this.currentCraftingPage][c];
			Item crafted = recipe.createItem();
			List<KeyValuePair<string, int>> seasoning = null;
			if (this.cooking && crafted.Quality == 0)
			{
				seasoning = new List<KeyValuePair<string, int>>();
				seasoning.Add(new KeyValuePair<string, int>("917", 1));
				if (CraftingRecipe.DoesFarmerHaveAdditionalIngredientsInInventory(seasoning, this.getContainerContents()))
				{
					crafted.Quality = 2;
				}
				else
				{
					seasoning = null;
				}
			}
			if (this.heldItem == null)
			{
				recipe.consumeIngredients(this._materialContainers);
				this.heldItem = crafted;
				if (playSound)
				{
					Game1.playSound("coin", null);
				}
			}
			else
			{
				if (!(this.heldItem.Name == crafted.Name) || !this.heldItem.getOne().canStackWith(crafted.getOne()) || this.heldItem.Stack + recipe.numberProducedPerCraft - 1 >= this.heldItem.maximumStackSize())
				{
					return;
				}
				this.heldItem.Stack += recipe.numberProducedPerCraft;
				recipe.consumeIngredients(this._materialContainers);
				if (playSound)
				{
					Game1.playSound("coin", null);
				}
			}
			if (seasoning != null)
			{
				if (playSound)
				{
					Game1.playSound("breathin", null);
				}
				CraftingRecipe.ConsumeAdditionalIngredients(seasoning, this._materialContainers);
				if (!CraftingRecipe.DoesFarmerHaveAdditionalIngredientsInInventory(seasoning, this.getContainerContents()))
				{
					Game1.showGlobalMessage(Game1.content.LoadString("Strings\\StringsFromCSFiles:Seasoning_UsedLast"));
				}
			}
			Game1.player.NotifyQuests((Quest quest) => quest.OnRecipeCrafted(recipe, crafted, false), false);
			if (!this.cooking && Game1.player.craftingRecipes.ContainsKey(recipe.name))
			{
				NetStringDictionary<int, NetInt> craftingRecipes = Game1.player.craftingRecipes;
				string name = recipe.name;
				craftingRecipes[name] += recipe.numberProducedPerCraft;
			}
			if (this.cooking)
			{
				Game1.player.cookedRecipe(this.heldItem.ItemId);
				Game1.stats.checkForCookingAchievements();
			}
			else
			{
				Game1.stats.checkForCraftingAchievements();
			}
			if (Game1.options.gamepadControls && this.heldItem != null && Game1.player.couldInventoryAcceptThisItem(this.heldItem))
			{
				Game1.player.addItemToInventoryBool(this.heldItem, false);
				this.heldItem = null;
			}
		}

		// Token: 0x060028A8 RID: 10408 RVA: 0x001DA960 File Offset: 0x001D8B60
		public override void receiveRightClick(int x, int y, bool playSound = true)
		{
			this.heldItem = this.inventory.rightClick(x, y, this.heldItem, true, false);
			foreach (ClickableTextureComponent c in this.pagesOfCraftingRecipes[this.currentCraftingPage].Keys)
			{
				if (c.containsPoint(x, y) && !c.hoverText.Equals("ghosted") && this.pagesOfCraftingRecipes[this.currentCraftingPage][c].doesFarmerHaveIngredientsInInventory(this.getContainerContents()))
				{
					this.clickCraftingRecipe(c, true);
				}
			}
		}

		// Token: 0x060028A9 RID: 10409 RVA: 0x001DAA20 File Offset: 0x001D8C20
		public override void performHoverAction(int x, int y)
		{
			CraftingRecipe prevHoverRecipe = this.hoverRecipe;
			base.performHoverAction(x, y);
			this.hoverTitle = "";
			this.hoverText = "";
			this.hoverRecipe = null;
			this.hoverItem = this.inventory.hover(x, y, this.hoverItem);
			this.hoverAmount = -1;
			if (this.hoverItem != null)
			{
				this.hoverTitle = this.inventory.hoverTitle;
				this.hoverText = this.inventory.hoverText;
			}
			foreach (ClickableTextureComponent c in this.pagesOfCraftingRecipes[this.currentCraftingPage].Keys)
			{
				if (c.containsPoint(x, y, 4))
				{
					if (c.hoverText.Equals("ghosted"))
					{
						this.hoverText = "???";
					}
					else
					{
						this.hoverRecipe = this.pagesOfCraftingRecipes[this.currentCraftingPage][c];
						if (prevHoverRecipe == null || prevHoverRecipe.name != this.hoverRecipe.name)
						{
							this.lastCookingHover = this.hoverRecipe.createItem();
						}
						c.scale = Math.Min(c.scale + 0.02f, c.baseScale + 0.1f);
					}
				}
				else
				{
					c.scale = Math.Max(c.scale - 0.02f, c.baseScale);
				}
			}
			if (this.upButton != null)
			{
				if (this.upButton.containsPoint(x, y))
				{
					this.upButton.scale = Math.Min(this.upButton.scale + 0.02f, this.upButton.baseScale + 0.1f);
				}
				else
				{
					this.upButton.scale = Math.Max(this.upButton.scale - 0.02f, this.upButton.baseScale);
				}
			}
			if (this.downButton != null)
			{
				if (this.downButton.containsPoint(x, y))
				{
					this.downButton.scale = Math.Min(this.downButton.scale + 0.02f, this.downButton.baseScale + 0.1f);
				}
				else
				{
					this.downButton.scale = Math.Max(this.downButton.scale - 0.02f, this.downButton.baseScale);
				}
			}
			if (this.trashCan != null)
			{
				if (this.trashCan.containsPoint(x, y))
				{
					if (this.trashCanLidRotation <= 0f)
					{
						Game1.playSound("trashcanlid", null);
					}
					this.trashCanLidRotation = Math.Min(this.trashCanLidRotation + 0.06544985f, 1.5707964f);
					if (this.heldItem != null && Utility.getTrashReclamationPrice(this.heldItem, Game1.player) > 0)
					{
						this.hoverText = Game1.content.LoadString("Strings\\UI:TrashCanSale");
						this.hoverAmount = Utility.getTrashReclamationPrice(this.heldItem, Game1.player);
						return;
					}
				}
				else
				{
					this.trashCanLidRotation = Math.Max(this.trashCanLidRotation - 0.06544985f, 0f);
				}
			}
		}

		// Token: 0x060028AA RID: 10410 RVA: 0x001DAD64 File Offset: 0x001D8F64
		public override bool readyToClose()
		{
			return this.heldItem == null;
		}

		// Token: 0x060028AB RID: 10411 RVA: 0x001DAD70 File Offset: 0x001D8F70
		public override void draw(SpriteBatch b)
		{
			if (this._standaloneMenu)
			{
				Game1.drawDialogueBox(this.xPositionOnScreen, this.yPositionOnScreen, this.width, this.height, false, true, null, false, true, -1, -1, -1);
			}
			base.drawHorizontalPartition(b, this.yPositionOnScreen + IClickableMenu.borderWidth + IClickableMenu.spaceToClearTopBorder + 256, false, -1, -1, -1);
			this.inventory.draw(b);
			if (this.trashCan != null)
			{
				this.trashCan.draw(b);
				b.Draw(Game1.mouseCursors, new Vector2((float)(this.trashCan.bounds.X + 60), (float)(this.trashCan.bounds.Y + 40)), new Rectangle?(new Rectangle(564 + Game1.player.trashCanLevel * 18, 129, 18, 10)), Color.White, this.trashCanLidRotation, new Vector2(16f, 10f), 4f, SpriteEffects.None, 0.86f);
			}
			b.End();
			b.Begin(SpriteSortMode.FrontToBack, BlendState.AlphaBlend, SamplerState.PointClamp, null, null, null, null);
			foreach (ClickableTextureComponent c in this.pagesOfCraftingRecipes[this.currentCraftingPage].Keys)
			{
				if (c.hoverText.Equals("ghosted"))
				{
					c.draw(b, Color.Black * 0.35f, 0.89f, 0, 0, 0);
				}
				else if (!this.pagesOfCraftingRecipes[this.currentCraftingPage][c].doesFarmerHaveIngredientsInInventory(this.getContainerContents()))
				{
					c.draw(b, Color.DimGray * 0.4f, 0.89f, 0, 0, 0);
					if (this.pagesOfCraftingRecipes[this.currentCraftingPage][c].numberProducedPerCraft > 1)
					{
						NumberSprite.draw(this.pagesOfCraftingRecipes[this.currentCraftingPage][c].numberProducedPerCraft, b, new Vector2((float)(c.bounds.X + 64 - 2), (float)(c.bounds.Y + 64 - 2)), Color.LightGray * 0.75f, 0.5f * (c.scale / 4f), 0.97f, 1f, 0, 0);
					}
				}
				else
				{
					c.draw(b);
					if (this.pagesOfCraftingRecipes[this.currentCraftingPage][c].numberProducedPerCraft > 1)
					{
						NumberSprite.draw(this.pagesOfCraftingRecipes[this.currentCraftingPage][c].numberProducedPerCraft, b, new Vector2((float)(c.bounds.X + 64 - 2), (float)(c.bounds.Y + 64 - 2)), Color.White, 0.5f * (c.scale / 4f), 0.97f, 1f, 0, 0);
					}
				}
			}
			b.End();
			b.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend, SamplerState.PointClamp, null, null, null, null);
			if (this.hoverItem != null)
			{
				IClickableMenu.drawToolTip(b, this.hoverText, this.hoverTitle, this.hoverItem, this.heldItem != null, -1, 0, null, -1, null, -1, null);
			}
			else if (!string.IsNullOrEmpty(this.hoverText))
			{
				if (this.hoverAmount > 0)
				{
					IClickableMenu.drawToolTip(b, this.hoverText, this.hoverTitle, null, true, -1, 0, null, -1, null, this.hoverAmount, null);
				}
				else
				{
					IClickableMenu.drawHoverText(b, this.hoverText, Game1.smallFont, (this.heldItem != null) ? 64 : 0, (this.heldItem != null) ? 64 : 0, -1, null, -1, null, null, 0, null, -1, -1, -1, 1f, null, null, null, null, null, null, 1f, -1, -1);
				}
			}
			Item item = this.heldItem;
			if (item != null)
			{
				item.drawInMenu(b, new Vector2((float)(Game1.getOldMouseX() + 16), (float)(Game1.getOldMouseY() + 16)), 1f);
			}
			base.draw(b);
			if (this.downButton != null && this.currentCraftingPage < this.pagesOfCraftingRecipes.Count - 1)
			{
				this.downButton.draw(b);
			}
			if (this.upButton != null && this.currentCraftingPage > 0)
			{
				this.upButton.draw(b);
			}
			if (this._standaloneMenu)
			{
				Game1.mouseCursorTransparency = 1f;
				base.drawMouse(b, false, -1);
			}
			if (this.hoverRecipe != null)
			{
				ObjectData rawHoverData;
				if (this.cooking && this.lastCookingHover.HasTypeObject() && Game1.objectData.TryGetValue(this.lastCookingHover.ItemId, out rawHoverData))
				{
					BuffEffects effects = new BuffEffects();
					foreach (Buff buff in Object.TryCreateBuffsFromData(rawHoverData, this.lastCookingHover.Name, this.lastCookingHover.DisplayName, 1f, new Action<BuffEffects>(this.lastCookingHover.ModifyItemBuffs)))
					{
						effects.Add(buff.effects);
					}
					if (effects.HasAnyValue())
					{
						effects.ToLegacyAttributeFormat();
					}
				}
				IClickableMenu.drawToolTip(b, " ", this.hoverRecipe.DisplayName + ((this.hoverRecipe.numberProducedPerCraft > 1) ? (" x" + this.hoverRecipe.numberProducedPerCraft.ToString()) : ""), this.lastCookingHover, this.heldItem != null, -1, 0, null, -1, this.hoverRecipe, -1, this.getContainerContents());
			}
		}

		// Token: 0x060028AC RID: 10412 RVA: 0x001DB374 File Offset: 0x001D9574
		protected override bool _ShouldAutoSnapPrioritizeAlignedElements()
		{
			return false;
		}

		// Token: 0x060028AD RID: 10413 RVA: 0x001DB378 File Offset: 0x001D9578
		public override bool IsAutomaticSnapValid(int direction, ClickableComponent a, ClickableComponent b)
		{
			return ((a != this.downButton && a != this.upButton) || direction != 3 || b.region == 8000) && (a.region != 8000 || (direction != 3 && direction != 1) || b.region != 9000) && (a.region != 8000 || direction != 2 || (b != this.upButton && b != this.downButton)) && base.IsAutomaticSnapValid(direction, a, b);
		}

		// Token: 0x060028AE RID: 10414 RVA: 0x001DB3FC File Offset: 0x001D95FC
		public override void emergencyShutDown()
		{
			base.emergencyShutDown();
			if (this.heldItem != null)
			{
				Item item = this.heldItem;
				this.heldItem = null;
				Utility.CollectOrDrop(item);
			}
		}

		// Token: 0x04001A48 RID: 6728
		public const int howManyRecipesFitOnPage = 40;

		// Token: 0x04001A49 RID: 6729
		public const int numInRow = 10;

		// Token: 0x04001A4A RID: 6730
		public const int numInCol = 4;

		// Token: 0x04001A4B RID: 6731
		public const int region_upArrow = 88;

		// Token: 0x04001A4C RID: 6732
		public const int region_downArrow = 89;

		// Token: 0x04001A4D RID: 6733
		public const int region_craftingSelectionArea = 8000;

		// Token: 0x04001A4E RID: 6734
		public const int region_craftingModifier = 200;

		// Token: 0x04001A4F RID: 6735
		public string hoverText = "";

		// Token: 0x04001A50 RID: 6736
		public Item hoverItem;

		// Token: 0x04001A51 RID: 6737
		public Item lastCookingHover;

		// Token: 0x04001A52 RID: 6738
		public InventoryMenu inventory;

		// Token: 0x04001A53 RID: 6739
		public Item heldItem;

		// Token: 0x04001A54 RID: 6740
		[SkipForClickableAggregation]
		public List<Dictionary<ClickableTextureComponent, CraftingRecipe>> pagesOfCraftingRecipes = new List<Dictionary<ClickableTextureComponent, CraftingRecipe>>();

		// Token: 0x04001A55 RID: 6741
		public int currentCraftingPage;

		// Token: 0x04001A56 RID: 6742
		public CraftingRecipe hoverRecipe;

		// Token: 0x04001A57 RID: 6743
		public ClickableTextureComponent upButton;

		// Token: 0x04001A58 RID: 6744
		public ClickableTextureComponent downButton;

		// Token: 0x04001A59 RID: 6745
		public bool cooking;

		// Token: 0x04001A5A RID: 6746
		public ClickableTextureComponent trashCan;

		// Token: 0x04001A5B RID: 6747
		public ClickableComponent dropItemInvisibleButton;

		// Token: 0x04001A5C RID: 6748
		public float trashCanLidRotation;

		// Token: 0x04001A5D RID: 6749
		public List<IInventory> _materialContainers;

		// Token: 0x04001A5E RID: 6750
		protected bool _standaloneMenu;

		// Token: 0x04001A5F RID: 6751
		public int hoverAmount;

		// Token: 0x04001A60 RID: 6752
		public List<ClickableComponent> currentPageClickableComponents = new List<ClickableComponent>();

		// Token: 0x04001A61 RID: 6753
		private string hoverTitle = "";
	}
}
